...
module.exports = class CatalogDAO {
	...
	static async getUserPets(currentOwner) {
		const userId = new ObjectId(currentOwner);
		const result = await petCollection.find(currentOwner.userId).toArray();
		// return await petCollection.find(currentOwner).toArray();
		return console.log('result', result['currentOwner']);
	}
