...
module.exports = class CatalogController {
	...
	static async updatePet(req, res) {
		...
		const cleanPetToUpdateObject = this.filterEmptyValues(petToUpdateObject);

		console.log('cleanPetToUpdateObject', cleanPetToUpdateObject);
		...
		try {
			...
			if (!petObj || currentUserId !== petOwnerId)
				return res.status(400).json({
					success: false,
					message: 'Pet or pet does not exist',
				});
