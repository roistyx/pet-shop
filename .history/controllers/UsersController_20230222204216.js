...
module.exports = class UsersController {
	...
	static async savePet(req, res) {
		...
		try {
			...
			console.log('petId', petObj.pets);

			if (petObj.pets.includes(petId)) console.log('Pet already saved');

			if (!petObj)
				return res.status(400).json({
					success: false,
					message: 'You are not the owner of this pet or pet does not exist',
				});
