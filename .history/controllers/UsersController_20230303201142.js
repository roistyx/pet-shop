...
const myFunction = async (savedPetId) => {
	const savedPets = await PetsDAO.getSavedPets(savedPetId);
};
module.exports = class UsersController {
	...
	static async getSavedPets(req, res) {
		try {
			const userSavedPetsArray = req.currentUser.pets;
			savedPets.map((pet) => {
				myFunction(pet._id);
				return console.log('Saved pet', pet);
			});
		} catch (err) {
	...
	static async savePet(req, res) {
		try {
			...
			// const pets = petObj['pets'];

			console.log('pets', petObj);

			// if (petList.includes(petId)) console.log('Pet already saved');
			...
			// const updatedUserPetsList = savedPet.pets;
			// console.log('temp', temp.pets);
			return res.status(200).json({
				success: true,
				message: 'Pet saved',
				// user: updatedUserObj,
	...
	static async unSavePet(req, res) {
		try {
			...
			// const updatedUserPetsList = updatedUserObj.pets;
			console.log('savedPet', updatedUserObj);
			return res.status(200).json({
				success: true,
				message: 'Pet unsaved',
				// pets: updatedUserObj,
