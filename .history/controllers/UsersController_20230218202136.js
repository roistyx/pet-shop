...
module.exports = class UsersController {
	...
	static async caregiverStatus(req, res) {
		...
		try {
			...
			if (petToUpdateObject.status === 'return_pet') {
				petToUpdateObject.currentOwner = null;
				await CatalogDAO.updatePetById(petId, petToUpdateObject);

				return res.status(200).json({
					success: false,
					message: 'Pet returned to agency',
					updatedPet: petToUpdateObject,
				});
			}
