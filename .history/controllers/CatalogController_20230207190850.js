...
module.exports = class CatalogController {
	static async Catalog(req, res) {
		try {
			...
			const exitingUser = await CatalogDAO.getUserByUsername(
				userObject.username
			);
			if (exitingUser) {
			...
			if (await CatalogDAO.getUserByUsername(userObject.username)) {
			...
			await CatalogDAO.createUser(userObject);
	...
	static async Login(req, res) {
		try {
			const user = await CatalogDAO.getUserByUsername(req.body.username);
	...
	static async Auth(req, res) {
		...
		try {
			...
			const user = await CatalogDAO.getUserByUsername(decoded.username);
