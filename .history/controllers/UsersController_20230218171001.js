...
module.exports = class UsersController {
	...
	static async savePet(req, res) {
		try {
			...
			const savedPetsList = await CatalogDAO.getPetCatalog(currentUserId);
			await UsersDAO.savePet(currentUserId, petId);
			console.log('savedPet', savedPetsList);
			return res.status(200).json({
				success: true,
				message: 'Pet saved',
				save_list: savedPetsList,
			});
		} catch (err) {
	...
	static async deletePet(req, res) {
		try {
			...
			savedList = await CatalogDAO.getPetCatalog(currentUserId);
