...
module.exports = class UsersController {
	...
	static async savePet(req, res) {
		try {
			...
			const returnedSavedList = await UsersDAO.savePet(currentUserId, petId);
			console.log('savedPet', returnedSavedList);
			return res.status(200).json({
				success: true,
				message: 'Pet saved',
				saved_list: returnedSavedList,
			});
		} catch (err) {
	...
	static async deletePet(req, res) {
		try {
			...
			const returnedSavedList = await CatalogDAO.getPetCatalog(currentUserId);

			console.log('returnedSavedList', returnedSavedList);
			return res.status(200).json({
				success: true,
				message: 'Pet deleted',
				save_list: returnedSavedList,
			});
		} catch (err) {
