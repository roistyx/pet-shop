...
} = require('./validations/petsValidations');
...
module.exports = class CatalogController {
	static async Catalog(req, res) {
		try {
			...
			const exitingpet = await CatalogDAO.getPetByID(petObject.petname);
			if (exitingpet) {
				return res.status(400).json({
					success: false,
					message: 'petname already exists',
				});
			}
			...
			if (await CatalogDAO.getPetByID(petObject.petname)) {
				console.log('petname already exists', petObject);
				return res.status(400).json({
					success: false,
					message: 'petname already exists',
				});
